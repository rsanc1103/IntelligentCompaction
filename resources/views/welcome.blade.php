        ...
        window.onload = () => {
          ...
          reader.onload = () => {
            ...
            for (let i = 0; i < csv.length-1; i++) {
              let element = csv[i+1].split(',');
              flightPath = {
                paths: [
                  { lat: Number(element[0]), lng: Number(element[4]) },
                  { lat: Number(element[1]), lng: Number(element[5]) },
                  { lat: Number(element[2]), lng: Number(element[6]) },
                  { lat: Number(element[3]), lng: Number(element[7]) },
                ],
                geodesic: true,
